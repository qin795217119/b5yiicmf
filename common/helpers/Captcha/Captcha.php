<?php
namespace common\helpers\Captcha;

use Yii;
class Captcha
{
    protected $config = [
        'seKey'    => 'b5net.com',// 验证码加密密钥
    ...
    {
        // 图片宽(px)
        $this->imageW || $this->imageW = 120;
        // 图片高(px)
        $this->imageH || $this->imageH = 40;
        ...
        $y = $this->imageH-round(($this->imageH - $this->fontSize) /2);
        $this->fontSize=round(($this->imageW/$this->length-$this->padding)/1.5);
        if ($this->useZh) {
            // 中文验证码
            ...
            for ($i = 0; $i < $this->length; $i++) {
                ...
                $codeNX += ($i==0)?$this->padding*2:$this->fontSize+$this->padding*2;
                imagettftext($this->_image, $this->fontSize, mt_rand(-40, 40), $codeNX, $y, $this->_color, $this->fontttf, $code[$i]);
            }
        }
        ...
        $response->send();
        //return ($content, 200, ['Content-Length' => strlen($content)])->contentType('image/png');
    }
    ...
    {
        for ($i = 0; $i < 4; $i++) {
            $linecolor = imagecolorallocate($this->_image, mt_rand(50, 200), mt_rand(50, 200), mt_rand(50, 200));
            imageline($this->_image, mt_rand(1, $this->imageW), mt_rand(1, $this->imageH), mt_rand(1, $this->imageW), mt_rand(1, $this->imageH), $linecolor);
        }
        return;
        $px = $py = 0;
    ...
    {
        for ($i = 0; $i < 200; $i++) {
            $pointcolor = imagecolorallocate($this->_image, mt_rand(50, 200), mt_rand(50, 200), mt_rand(50, 200));
            imagesetpixel($this->_image, mt_rand(1, $this->imageW), mt_rand(1, $this->imageH), $pointcolor);
        }
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            for ($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring($this->_image, 3, mt_rand(-10, $this->imageW), mt_rand(-10, $this->imageH), $codeSet[mt_rand(0, 29)], $noiseColor);
            }
        }
    }
