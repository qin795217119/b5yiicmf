...
<script>
    $(function () {
        b5uploadLink("<?=$widget_data['id']?>");
        $("#<?=$widget_data['id']?>").click(function () {
            var url = "<?=\yii\helpers\Url::toRoute(['common/cropper'])?>";
            var params = "id=<?=$widget_data['id']??''?>&name=<?=$widget_data['name']??''?>&cat=<?=$widget_data['cat']??''?>&water=<?=$widget_data['water']??''?>";
