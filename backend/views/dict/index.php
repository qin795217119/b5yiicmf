...
<?php $this->beginBlock('script'); ?>
    <script>
                ...
                columns: [
                    ...
                    {field: 'name', title: '字典名称'},
                    {
                        ...
                        formatter: function(value, row, index) {
                            return '<a href="javascript:void(0)" onclick="detail_mine(\'' + row.id + '\')">' + value + '</a>';
                        }},
                    ...
                    {field: 'note', title: '备注',visible: false},
                    {
                        ...
                        align: 'center',
                        formatter: function(value, row, index) {
                            return '<?=$this->render("/iframe",["name"=>"formopbtn","extend"=>["type"=>["edit"],"rowId"=>"row.id"]])?><?=$this->render("/iframe",["name"=>"formopbtn","extend"=>["type"=>["list_mine"],"rowId"=>"row.id"]])?><?=$this->render("/iframe",["name"=>"formopbtn","extend"=>["type"=>["delete"],"rowId"=>"row.id"]])?>';
                        }
                    }
                ]
            };
        ...
        function detail_mine(dictId) {
            ...
            url=urlcreate(url,'parent_id=' + dictId);
